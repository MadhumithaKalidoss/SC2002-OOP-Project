...
public class ReportGenerationService {
    ...
    private void createRowForAttendeeDetails(Sheet sheet, int rowIndex, Camp camp) {
        ...
        for (CampCommittee committeeMember : camp.getListofCampCommittee()) {
            // Access the user ID and faculty directly from the CampCommittee (which is a Student)
            ...
            row.createCell(1).setCellValue(userId); // User ID of the committee member
            row.createCell(2).setCellValue(faculty); // Faculty information
        }
